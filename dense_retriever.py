    ...
    with torch.no_grad():
        for j, batch_start in enumerate(range(0, n, bsz)):
            ...
            if True:
                ...
                #     ]
                # print("=" * 80)
                tmp_query_token = "<sent>"
                batch_token_tensors = [
                    tensorizer.text_to_tensor(" ".join([tmp_query_token, q]))
                    for q in batch_questions
                ]
            else:
                batch_token_tensors = [
...
class LocalFaissRetriever(DenseRetriever):
    ...
    def index_encoded_data(
        ...
        """
        buffer = []
        # print("Shape pf vector files ", len(vector_files))
        total_feature_buf = []
        # assert 0 == 1
        for i, item in enumerate(
            ...
            if 0 < buffer_size == len(buffer):
                ...
                total_feature_buf.extend(buffer)
                buffer = []
        self.index.index_data(buffer)
...
def validate(
    ...
    logger.info("Top-5: %.5f", top_k_hits[4])
    logger.info("Top-20: %.5f", top_k_hits[19])
    logger.info("Top-100: %.5f", top_k_hits[99])
...
def save_results(
    ...
    for i, q in enumerate(questions):
        ...
        docs = [passages[str.split(doc_id, "_")[0]] for doc_id in results_and_scores[0]]
