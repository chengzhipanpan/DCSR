...
BiEncoderPassage = collections.namedtuple("BiEncoderPassage", ["text", "title", "sent_labels"])

def filter_data(data_sample):
    # Filter those cases
    # Yes, previous filtering gives us much advantage in this setting now!
    positive_ctxs = data_sample["positive_ctxs"]
    negative_ctxs = data_sample["negative_ctxs"]
    hard_negative_ctxs = data_sample["hard_negative_ctxs"]
    good_flag = False
    for ctx in positive_ctxs:
        if 1 in ctx["sent_labels"]:
            good_flag = True
    if len(negative_ctxs) + len(hard_negative_ctxs) < 1:
        good_flag = False
    return good_flag
...
class RepSpecificTokenSelector(RepTokenSelector):
    def __init__(self, token: str = "<sent>"):
        self.token = token
        self.token_id = None
...
class SentTokenSelector(RepTokenSelector):
    def __init__(self, token: str = "<sent>"):
        self.token = token
        self.token_id = None
    ...
    def get_positions(self, input_ids: T, tenzorizer: Tensorizer):
        ...
        token_indexes = (input_ids == self.token_id).nonzero()
        token_indexes = token_indexes[:, 1].view(-1)

        token_indexes_result = []
        found_idx_cnt = 0
        # print(token_indexes.shape)      # why it is 7 * 2? print more information ehre!

        token_indexes = token_indexes.to(input_ids.device).view(-1)

        return_token_indexes = - torch.ones(10, dtype=torch.long)

        if len(token_indexes) >= 10:
            return_token_indexes = token_indexes[:10]
        else:
            return_token_indexes[:len(token_indexes)] = token_indexes
    
        return_token_indexes = return_token_indexes.view(-1)
        
        assert len(return_token_indexes) == 10
        
        return return_token_indexes
...
class JsonQADataset(Dataset):
    ...
    def load_data(self):
        ...
        self.data = [r for r in data if filter_data(r)]
    ...
    def __getitem__(self, index) -> BiEncoderSample:
        ...
        def create_passage(ctx: dict):
            return BiEncoderPassage(
                normalize_passage(ctx["new_text"]) if self.normalize else ctx["new_text"],
                ctx["title"],
                ctx["sent_labels"]
            )
...
class JsonLTablesQADataset(Dataset):
    ...
    def load_data(self):
        ...
        # filter those without positive ctx
        self.data = [r for r in data if (len(r["positive_ctxs"]) > 0 and (len(r["negative_ctxs"])+len(r["hard_negative_ctxs"] > 0)))]
        logger.info("Total cleaned data size: {}".format(len(self.data)))
