...
def check_answer(questions_answers_docs, tokenizer, match_type) -> List[bool]:
    ...
    true_doc_id = []
    # add temperature, normalization, and finally aggregation.
    ...
    if sent_mode:
        # Normalize the sentence scores for each document
        # And retrieve the top 100 ranked documents.
        true_doc_scores = {}

        doc_scores = torch.tensor(doc_scores, dtype=torch.float)
        doc_scores = torch.softmax(doc_scores, dim=-1).numpy()
        # doc_scores = np.softmax(doc_score, axis=-1)
        doc_scores = 1 - doc_scores
        for i, (doc_id, doc_score) in enumerate(zip(doc_ids, doc_scores)):
            cur_doc_id = str.split(doc_id, sep="_")[0]
            
            if cur_doc_id in true_doc_scores.keys():
                true_doc_scores[cur_doc_id] *= float(doc_score)
            else:
                true_doc_scores[cur_doc_id] = float(doc_score)
        
        for k in sorted(true_doc_scores, key=true_doc_scores.__getitem__):
            doc = dpr_all_documents[str.split(k, sep="_")[0]]
            text = doc[0]
            ...
            true_doc_id.append(cur_doc_id)
            if len(true_doc_id) > 100:
                break
    else:
        for i, doc_id in enumerate(doc_ids):
            cur_doc_id = str.split(doc_id, sep="_")[0]
            if cur_doc_id in true_doc_id:
                continue
            # assert doc_id[-1] == "3"
            doc = dpr_all_documents[str.split(doc_id, sep="_")[0]]
            text = doc[0]
