...
class HFBertEncoder(BertModel):
    ...
    def forward(
        ...
        if batch_sent_positions is not None:   # bs_size * 10
            bs_size = len(batch_sent_positions)
            sent_num = len(batch_sent_positions[0])

            assert sent_num == 10

            sent_token_emb = torch.zeros([bs_size, sent_num, pooled_output.size(-1)], dtype=torch.float).to(sequence_output.device)
            choose_token_idxs = -torch.ones([bs_size, sent_num], dtype=torch.long).to(sequence_output.device)
            # true_sent_num = 0
            for b_id in range(bs_size):
                true_sent_num = 0
                for s_id in range(sent_num):
                    if batch_sent_positions[b_id][s_id] >= 0:
                        true_sent_num += 1
                        sent_token_emb[b_id][s_id] = sequence_output[b_id][batch_sent_positions[b_id][s_id]]
                for j in range(true_sent_num):
                    choose_token_idxs[b_id][j] = 1
                   
            return choose_token_idxs, sequence_output, sent_token_emb, hidden_states
