...
from dpr.models.biencoder import BiEncoder, BiEncoderNllLoss, BiEncoderBatch, BiEncoderSentBatch
...
def _do_biencoder_fwd_pass(
    ...
    cfg,
    encoder_type: str,
    rep_positions=1, # <sent token>
    loss_scale: float = None,
) -> Tuple[torch.Tensor, int]:

    input = BiEncoderSentBatch(**move_to_device(input._asdict(), cfg.device))
    ...
    if model.training:
        model_out = model(
            ...
            input.ctx_segments,
            ctx_attn_mask,
            input.ctx_sent_labels,
            input.ctx_sent_start_ids,
            encoder_type=encoder_type,
            representation_token_pos=rep_positions,
        )
    else:
