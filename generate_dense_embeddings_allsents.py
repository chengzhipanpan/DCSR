...
from dpr.data.biencoder_data import BiEncoderPassage, SentTokenSelector
...
def gen_ctx_vectors(
    ...
    total = 0
    results = []
    sent_selector = SentTokenSelector()
    for j, batch_start in enumerate(range(0, n, bsz)):
        ...
        batch_sent_positions = [
            sent_selector.get_positions(input_tensor.unsqueeze(0), tensorizer)
            for input_tensor in batch_token_tensors
        ]
        batch_sent_positions = torch.cat([sent_p.view(1, -1) for sent_p in batch_sent_positions], dim=0)
        # print("Showing the shape of b batch sent positions", batch_sent_positions)
        batch_sent_positions.to(cfg.device)
        ...
        with torch.no_grad():
            choose_idxs, _, out, _ = model(ctx_ids_batch, ctx_seg_batch, ctx_attn_mask, batch_sent_positions=batch_sent_positions)
        out = out.cpu()
        choose_idxs = choose_idxs.cpu()
        ...
        if extra_info:
            results.extend(
                [
                    (ctx_ids[i]+"_"+str(j), out[i][j].view(-1).numpy(), *extra_info[i])
                    for i in range(out.size(0))
                    for j in range(out.size(1))
                    if choose_idxs[i][j] == 1
                ]
            )
        else:
            extender = [
                    (ctx_ids[i]+"_"+str(j), out[i][j].view(-1).numpy())
                    for i in range(out.size(0))
                    for j in range(out.size(1))
                    if choose_idxs[i][j] == 1
            ]
